...
class Collaborate_Filter:
    def __init__(self, input_file_name, user_id, k):
    ...
    def initialize(self):
        ...
        # load data
        self.dataset, self.uu_dataset = self.load_data(self.input_file_name)
        #print(self.dataset)
        #print()
        #print()
        #print()
        #print(self.uu_dataset)
        #print()
        #print()
        #print()
        #print(self.ii_dataset)
    ...
    def user_average_rating(self, user_data):
        avg_rating = 0.0
        size = len(user_data)
        for (rest, rating) in user_data.items():
    ...
    def common_items(self, user1_data, user2_data):
        result = []
        ht = {}
        for (rest, rating) in user1_data.items():
            ht.setdefault(rest, 0)
            ht[rest] += 1
        for (rest, rating) in user2_data.items():
            ht.setdefault(rest, 0)
            ht[rest] += 1
        for (k, v) in ht.items():
            if v == 2:
    ...
    def display(self, k_nearest_neighbors):
        for neighbor in k_nearest_neighbors:
...
cf = Collaborate_Filter(input_file_name, user_id, k)
cf.initialize()
...
k_nearest_neighbors = cf.k_nearest_neighbors(user_id, k)
    ...
    # cf.display(k_nearest_neighbors)

print(json.dumps(k_nearest_neighbors))
